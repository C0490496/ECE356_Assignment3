...
void *airplane_deadlock_prone(void* arg){
	int id = *(int *)arg;
	while(1){
		printf("Airplane %d: Requesting to runway...\n", id);
		sem_wait(&runways);	//Locking a runway using semaphore.
...
int main(){
	...
	for(int i=0; i<NUM_AIRPLANES; i++){
		airplane_ids[i] = i+1;
		pthread_create(&airplanes[i], NULL, airplane_deadlock_prone, &airplane_ids[i]);
	}
